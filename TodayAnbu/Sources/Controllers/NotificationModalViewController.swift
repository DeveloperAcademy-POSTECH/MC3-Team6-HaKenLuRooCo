...
class NotificationModalViewController: UIViewController {
    ...
    let settingDoneButton: UIButton = {
        ...
        button.addTarget(NotificationModalViewController.self, action: #selector(settingDoneButtonAction(_:)), for: .touchUpInside)
        return button
    }()
    ...
    private func settingButtonLayOut() {
        view.addSubview(settingDoneButton)
    ...
    @objc func settingDoneButtonAction(_: UIButton!) {
        dismiss(animated: true)
        print("버튼이 선택되었음")
    }

    // 알람 설정 버튼과 연동해야함, Completion Feedback(진동,소리)은 버튼 애니메이션과 파란색 활성화로 대체
    @IBAction func setAlarmSchedule(_: Any) { notificationCenter.getNotificationSettings { settings in
